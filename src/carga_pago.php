...
<div class="panel-heading">
			<h6 class="panel-title" style="position: initial !important;">
				...
				?>
				<a href="datatable_pago.php"><i class='icon-arrow-left7'></i></a> <b> <?php echo utf8_encode($_SESSION["tipo_ls"].': '.$_SESSION['user_ls'])  ?><br/> Listado de Cargas carga_PAGO.csv</b>
			</h6>
</div>
...
$nombre_archivo = "assets/archivos/carga_pago.csv";
if (file_exists($nombre_archivo)) {
    $fecha_modifica= date("Y-m-d h:i:s", filemtime($nombre_archivo));
	$fecha_actual= date("Y-m-d h:i:s");
	$sql=mysqli_query($con,"select * from carga_log where fecha_modificacion ='$fecha_modifica' and tipo='PAGO'");
    $cant=mysqli_num_rows($sql);
	//echo $cant;
	if ($cant==0 AND utf8_encode($_SESSION["tipo_ls"])=="ADMIN"){
    $sql="insert into carga_log (id, fecha_modificacion, fecha_carga,tipo) values(default,'$fecha_modifica','$fecha_actual','PAGO')";
    ...
    while (($datos=fgetcsv($info,5000,",")) !== FALSE){
      ...
      $linea[]=array('nombre'=>$datos[0],'cartera'=>$datos[1],'identificador'=>$datos[2],'tipo'=>$datos[3],'fecha_pago'=>$exd_new,'hora_pago'=>$datos[5],'monto'=>$datos[6],'homolo'=>$datos[7]);
	  }
   ...
   foreach($linea as $indice=>$value){
	  if($value["nombre"]!="nombre"){
		  ...
		  $tipo=$value["tipo"];
		  $fecha_pago=$value["fecha_pago"].' '.$value["hora_pago"];
		  $monto=$value["monto"];
		  $homolo=$value["homolo"];
		  /*$character = array("'",'"');
		  ...
		  $sql=mysqli_query($con,"select * from pagos where identificador ='$identificador'  and tipo='$tipo' and IDESTADO=1");
		  $num=mysqli_num_rows($sql);
		  if ($num==0){
			 $sql="insert into pagos (idcarga,pagos,IDCARTERA,IDENTIFICADOR,TIPO,FECHAPAG,MONTO,HOMOLO,IDESTADO) values ($id_carga,'$nombre','$cartera','$identificador','$tipo','$fecha_pago','$monto','$homolo',1)";
			 //echo $sql;
			 ...
			 $sql="update pagos set IDESTADO=0 where identificador='$identificador' and tipo='$tipo' ";
		...
		mysqli_query($con,$sql);
		$sql="update pagos set FECHAREG='$fecha_actual' where idcarga= $id_carga";
		mysqli_query($con,$sql);	
...
<table id="deporte" class="table table-hover" border="0" align="center" style="font-size:12px;">
	...
	<?php
		...
		$var = clsCarga::listar_cargas('PAGO');
