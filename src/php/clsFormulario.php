...
class clsFormulario{
    ...
    public static function distribuidor(){
		$objConx = new clsConexion();
		$objConx->conectar();
		$sql = "SELECT id as id,upper(nombre) as nombre FROM distribuidor
		        order by nombre asc";
		$res = mysql_query($sql) or die(mysql_error());
		$arr_datos = array();
		while($row = mysql_fetch_array($res)) {
	...
	public static function tipis(){
		$objConx = new clsConexion();
		$objConx->conectar();
		$sql = "SELECT id as id,upper(nombre) as nombre FROM tipi
		        order by nombre asc";
		$res = mysql_query($sql) or die(mysql_error());
		$arr_datos = array();
		while($row = mysql_fetch_array($res)) {
	...
	public static function dealers($id){
		$objConx = new clsConexion();
		$objConx->conectar();
		$sql = "SELECT id as id,upper(nombre) as nombre FROM dealer 
	...
	public static function tipi2($id){
		$objConx = new clsConexion();
		$objConx->conectar();
		$sql = "SELECT id as id,upper(nombre) as nombre FROM tipi2 
		where id_tipi=$id 
		order by nombre asc";
		$res = mysql_query($sql) or die(mysql_error());
		$arr_datos = array();
	...
	public static function consulta_horario(){
		$objConx = new clsConexion();
		$objConx->conectar();
		$sql = "SELECT a.idhorario as id,concat(a.horainicio,'-',a.horafin,' | ',b.nombre,' | ',a.horario) as nombre FROM horario a
	...
	public static function registrar_empleado($APELLIDOS,$NOMBRES,$FECHANAC,$SEXO,$DOC,$ESTCIV,$CARFAM,$NUMHIJ,$DIRECCION,$DISTRITO,$DPTO,$REFDIR,$TLF,$CEL,$EMAIL,$GRADOINS,$CARGO,$IDSUCURSAL,$USUARIO,$PASSWORD,$cartera) {
		$objConx = new clsConexion();
		$objConx->conectar();
		$fecha=date("Y-m-d H:i:s");
		$sql = "INSERT INTO personal VALUES(default,upper('$APELLIDOS'),upper('$NOMBRES'),'$FECHANAC','$SEXO','$DOC','$ESTCIV','$CARFAM','$NUMHIJ','$DIRECCION','$DISTRITO','$DPTO','$REFDIR','$TLF','$CEL','$EMAIL','$GRADOINS','$CARGO','$IDSUCURSAL',upper('$USUARIO'),md5('$PASSWORD'),1,'$fecha','0000-00-00 00:00:00',$cartera)";
		//echo($sql);
		$res = mysql_query($sql) or die(mysql_error());
		$res = mysql_insert_id();
		$objConx->desconectar();
	...
	public static function listar_user() {
		$objConx = new clsConexion();
		$objConx->conectar();
		$sql = "SELECT CONCAT(YEAR(fecha_registro),'-', LPAD(b.idpersonal,5,'0') ) as idpersonal,concat(nombres,' ',apellidos) as empleado,b.DOC as dni,b.password,usuario as user,c.nombre as tipo, if(IDESTADO=1,'<label>ACTIVE</label>','<label>SUSPENDED</label>') as estado,'' as opciones FROM personal b left join cargo c on c.id=b.CARGO order by b.IDPERSONAL desc
		";
		$res = mysql_query($sql) or die(mysql_error());
		//echo($sql);
		$arr_datos = array();
		while($row = mysql_fetch_array($res)) {  
			  $arr_datos[] = array($row["idpersonal"],utf8_encode($row["empleado"]),$row["dni"],utf8_encode($row["password"]),utf8_encode($row["user"]),$row["tipo"],$row["estado"],$row["opciones"]);
		}
		$objConx->desconectar();
		if ($res)
		return $arr_datos;
	...
	public static function update_empleado($id,$estado,$APELLIDOS,$NOMBRES,$FECHANAC,$SEXO,$DOC,$ESTCIV,$CARFAM,$NUMHIJ,$DIRECCION,$DISTRITO,$DPTO,$REFDIR,$TLF,$CEL,$EMAIL,$GRADOINS,$CARGO,$IDSUCURSAL,$USUARIO,$PASSWORD,$cartera) {
		$objConx = new clsConexion();
		$objConx->conectar();
		$fecha=date("Y-m-d H:i:s");
		if($PASSWORD=="SI"){
			$sql = "UPDATE personal set IDESTADO=$estado,APELLIDOS=upper('$APELLIDOS'),NOMBRES=upper('$NOMBRES'),FECHANAC='$FECHANAC',SEXO='$SEXO',DOC='$DOC',ESTCIV=$ESTCIV,CARFAM='$CARFAM',NUMHIJ=$NUMHIJ,DIRECCION='$DIRECCION',DISTRITO='$DISTRITO',DPTO='$DPTO',REFDIR='$REFDIR',TLF='$TLF',CEL='$CEL',EMAIL='$EMAIL',GRADOINS='$GRADOINS',CARGO=$CARGO,IDSUCURSAL=$IDSUCURSAL,USUARIO='$USUARIO',id_cartera=$cartera WHERE idpersonal =$id";
		}else{
			$sql = "UPDATE personal set IDESTADO=$estado,APELLIDOS=upper('$APELLIDOS'),NOMBRES=upper('$NOMBRES'),FECHANAC='$FECHANAC',SEXO='$SEXO',DOC='$DOC',ESTCIV=$ESTCIV,CARFAM='$CARFAM',NUMHIJ=$NUMHIJ,DIRECCION='$DIRECCION',DISTRITO='$DISTRITO',DPTO='$DPTO',REFDIR='$REFDIR',TLF='$TLF',CEL='$CEL',EMAIL='$EMAIL',GRADOINS='$GRADOINS',CARGO=$CARGO,IDSUCURSAL=$IDSUCURSAL,USUARIO='$USUARIO',PASSWORD=md5('$PASSWORD'),id_cartera=$cartera  WHERE idpersonal =$id";
		}
		//echo($sql);
		$res = mysql_query($sql) or die(mysql_error());
		$res = mysql_insert_id();
