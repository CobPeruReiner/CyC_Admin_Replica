	...
	if (!isset($_SESSION['id_ls']) || !isset($_SESSION['user_ls']) || !isset($_SESSION['nombre_ls'])) {
		...
		if (sizeof($verificar) == 0) {
			...
			if ($_REQUEST['control'] == 1) {
				$responce->codigo = 1;
				$responce->mensaje = 'Motivo Registrado';

				clsMotivo::registrar(utf8_decode($_REQUEST['nombre']), $_REQUEST['efecto'], utf8_decode($_REQUEST['homologo']), utf8_decode($_REQUEST['descripcion']));
			} else if ($_REQUEST['control'] == 2) {
				$arr_datos = clsMotivo::listar();
				if (sizeof($arr_datos) > 0) {
					$responce->codigo = 1;
					$responce->arr_datos = $arr_datos;
					$responce->mensaje = 'Listado Motivo';
				} else {
				...
				if (isset($_REQUEST['id'])) {
					...
					clsMotivo::update_motivo($_REQUEST['id'], utf8_decode($_REQUEST['nombre']), $_REQUEST['efecto'], utf8_decode($_REQUEST['homologo']), utf8_decode($_REQUEST['descripcion']), $_REQUEST['estado']);
					$responce->mensaje = 'Actualizar Motivo';
				}
			} else {
				$responce->codigo = 1;
				$responce->mensaje = 'Eliminado Motivo';
				clsMotivo::baja($_REQUEST['id']);
			}
		}
	}
}
