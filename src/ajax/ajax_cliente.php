	...
	if (!isset($_SESSION['id_ls']) || !isset($_SESSION['user_ls']) || !isset($_SESSION['nombre_ls'])) {
		...
		if (sizeof($verificar)==0){
			...
			if ($_REQUEST['control']==1){
				$responce->codigo = 1;
				$responce->mensaje = 'Cliente Registrado';
				clsCliente::registrar(utf8_decode($_REQUEST['nombre']),utf8_decode($_REQUEST['identificador']),$_REQUEST['idpersonal']);
			  
			}else if($_REQUEST['control']==2 ){
		    	$arr_datos = clsCliente::listar();
            	if (sizeof($arr_datos)>0) {
            		$responce->codigo = 1;
            		$responce->arr_datos = $arr_datos;
            		$responce->mensaje='Listado Cliente';
            	}else{
			    ...
			    if (isset($_REQUEST['id'])){
		    	    ...
		    	    clsCliente::update($_REQUEST['id'],utf8_decode($_REQUEST['nombre']),utf8_decode($_REQUEST['identificador']),$_REQUEST['idpersonal'],$_REQUEST['estado']);
		    	    $responce->mensaje='Actualizar Cliente';
			    }
			}else{
				$responce->codigo = 1;
				$responce->mensaje = 'Eliminado';
				clsCliente::baja($_REQUEST['id']);
			}
