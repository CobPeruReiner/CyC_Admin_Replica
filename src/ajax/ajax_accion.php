	...
	if (!isset($_SESSION['id_ls']) || !isset($_SESSION['user_ls']) || !isset($_SESSION['nombre_ls'])) {
		...
		if (sizeof($verificar)==0){
			...
			if ($_REQUEST['control']==1){
				$responce->codigo = 1;
				$responce->mensaje = 'Accion Registrado';
				clsAccion::registrar(utf8_decode($_REQUEST['nombre']),$_REQUEST['cartera'],$_REQUEST['tipo']);
				
			}else if($_REQUEST['control']==2 ){
		    	$arr_datos = clsAccion::listar();
            	if (sizeof($arr_datos)>0) {
            		$responce->codigo = 1;
            		$responce->arr_datos = $arr_datos;
            		$responce->mensaje='Listado Accion';
            	}else{
			    ...
			    if (isset($_REQUEST['id'])){
			        $responce->codigo = 1;					
		    	    clsAccion::update_accion($_REQUEST['id'],$_REQUEST['nombre'],$_REQUEST['cartera'],$_REQUEST['tipo'],$_REQUEST['estado']);
		    	    $responce->mensaje='Actualizar Accion';
			    }
			}else{
				$responce->codigo = 1;
				$responce->mensaje = 'Eliminado Accion';
				clsAccion::baja($_REQUEST['id']);
			}
